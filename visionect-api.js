...
function removeDevice(id) {
  return methods.visionectDeleteMethod(
    "api/device/" + id,
    204,
    "application/json"
  );
...
function rebootDevice(id) {
  return methods.visionectPostMethod(
    "api/device/" + id,
    204,
    "application/json"
  );
...
function getServerLiveView(uuid) {
  return methods.visionectGetMethod(
    "/api/live/device/" + uuid + "/cached.png",
    200,
    "image/png"
  );
}
...
function getDeviceLiveView(uuid) {
  return methods.visionectGetMethod(
    "/api/live/device/" + uuid + "/image.png",
    200,
    "image/png"
  );
}
...
function setConfig(config) {
  ...
  helper.setProtocol(config.protocol || "http");
